...
class GitHubClient(GitProvider):
    ...
    async def obtener_documentacion(
        ...
        if clave_cache in cache_docs:
            ...
            if ahora - ts_guardado < ttl_segundos:
                if log:
                    log.info(f"[llm_wiki_assistant] Usando caché en memoria para documentación (filtro: '{filtro}')")
    ...
    async def _recorrer_carpeta_docs(
        ...
        async with semaforo:
            if log:
                log.info(f"[llm_wiki_assistant] Consultando GitHub: {url}")
    ...
    async def _descargar_fichero(
        ...
        async with semaforo:
            if log:
                log.info(f"[llm_wiki_assistant] Descargando archivo GitHub: {path}")
...
class GitLabClient(GitProvider):
    ...
    async def _recorrer_carpeta_docs(
        ...
        async with semaforo:
            if log:
                log.info(f"[llm_wiki_assistant] Consultando GitLab árbol: {url}")
            while True:
                params["page"] = page
                async with session.get(url, headers=headers, params=params) as resp:
                    if resp.status != 200:
    ...
    async def _descargar_fichero(
        ...
        async with semaforo:
            if log:
                log.info(f"[llm_wiki_assistant] Descargando archivo GitLab: {path}")
            async with session.get(url, headers=headers) as resp:
                if resp.status == 200:
                    contenido = await resp.text()
                    return f"## Archivo: {path}\n{contenido}"
