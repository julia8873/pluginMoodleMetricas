...
Uso:
  docker exec --user daemon maubot python3 -m llm_wiki_assistant.ejecutar_ingesta_lotes "raw/00Libro de Teoria Musical - Nestor Crespo-1784644777.md"
"""
import asyncio
import base64
import sys
...
async def main():
    if len(sys.argv) < 2:
        print("Uso: python3 -m llm_wiki_assistant.ejecutar_ingesta_lotes <ruta_raw>")
        sys.exit(1)
    ...
    if os.path.exists(db_path):
        ...
        try:
            ...
            row = conn.execute("SELECT config FROM instance WHERE id='dev.julia.llmwikiassistant'").fetchone()
            if row and row[0]:
                yaml = YAML()
